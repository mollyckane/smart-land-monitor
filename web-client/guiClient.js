...
function discoverServices() {
    ...
    services.forEach((service) => {
        ...
        namingClient.Lookup({ name: serviceName }, (err, res) => {
            if (err) {
                console.log(`[GUI] Lookup error for ${serviceName}: ${err.message}`);
                return;
            }
            if (!res.found) {
                console.log(`[GUI] Could not find ${serviceName}...check if it is running`);
                return;
            }
            try {
                ...
                console.log(`[GUI] Discovered ${serviceName} at ${res.address}`);
            }
            catch (error) {
                console.log(`[GUI] Failed to create client for ${serviceName}: ${error.message}`);
            }
        });
    });
}
...
app.listen(8080, () => {
    console.log('[GUI] Dashboard running at http://localhost:8080');
    console.log('[GUI] Discovering services...');
    discoverServices();
});
